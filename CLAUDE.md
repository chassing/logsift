...
│   ├── keybindings.py          # keybinding registry, normalization, validation
...
│   ├── colors.py               # color assignment for components and search
│   ├── export.py               # filtered log line export
...
│   │   ├── navigation_dialog.py # tabbed dialog for search, go-to-line, jump-to-time
...
│   │   ├── annotation_dialog.py # bookmark annotation input dialog
│   │   ├── export_dialog.py    # export options dialog (visible/all/bookmarked)
│   │   ├── timestamp_input.py  # timestamp input widget (natural language support)
...
| Model         | Purpose                                                                                                                                                                                                             |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LogLine`     | Single parsed log line: `line_number`, `raw`, `timestamp`, `content_type` (JSON/TEXT), `content` (computed), `content_offset`, `parsed_json`, `log_level`, `component`, `source_line_number`                        |
| `FilterRule`  | A filter: `filter_type` (INCLUDE/EXCLUDE), `pattern`, `enabled`, `is_regex`, `case_sensitive`, `is_json_key`, `json_key`, `json_value`, `is_component`, `component_name`, `is_time_range`, `time_start`, `time_end` |
| `SearchQuery` | Search: `pattern`, `case_sensitive`, `is_regex`, `direction` (FORWARD/BACKWARD)                                                                                                                                     |
| `AppConfig`   | Persisted config: `theme`, `keybindings`                                                                                                                                                                            |
| `Session`     | Named session: `name`, `filters`, `bookmarks`, `source_files`, `search_patterns`, `search_history`, `version`, `created_at`, `updated_at`                                                                           |
...
### Keybinding System (`keybindings.py`)

- **`DEFAULT_BINDINGS`**: dict mapping action IDs to default keys (source of truth for app-level bindings)
- **`LOGVIEW_BINDINGS`**: dict mapping action IDs to default keys (source of truth for log view bindings)
- **`normalize_key()`** / **`normalize_keybindings()`**: handle user key aliases (e.g. `ctrl+f` → `ctrl+f`)
- **`validate_keybindings()`**: validates user overrides (unknown actions, duplicate keys, reserved keys)
- **`build_keymap()`**: merges user overrides with defaults into a Textual keymap
- **`generate_defaults_toml()`**: generates TOML template for CLI `keybindings` command
- **`get_merged_bindings()`**: merges user config with defaults for help screen display
- **`format_key_display()`**: human-readable key formatting
...
- **Keybindings command**: `logdelve keybindings` — prints default keybindings as TOML template
...
5. **Docs**: Update `docs/guide.md` with any new features or changes, ensuring it matches the README and actual app behavior
...
These files must stay in sync when features change:
...
- ALWAYS keep the main documentation (README.md, docs/guide.md, AGENTS.md/CLAUDE.md) up to date with actual code!!!
